...
# 2b. apply threshold to extract CSF voxels (0 - 20HU)
# 2c. isolate narrowed region of CSF to limit subarachnoid CSF in next steps
...
# 2. open terminal window, execute 'bash SNImagingLab_STEP2_ct_segmentation.sh'
...
omat_dir=$image_dir/omat
#location of the bigvent and lilvent image files
interim_vent_dir=$image_dir/interim_vent
...
stdorient_dir=$image_dir/stdorient
...
##modified supratentorial mask to target ventricular csf and minimize subarachnoid CSF caputured by ventriculomegaly mask
...
##large third ventricle mask to REMOVE third ventricle contamination from lateral ventricle segmentation
...
##################################################################################################
fslmaths ${work_dir}/${fname_short}_suptent_ero1 -uthr 20 -bin ${work_dir}/${fname_short}_suptent_ero1_csf_sri
...
#step 9b. - cluster threshold 0.5 (include some volume averaged edges)
#range of input _bignlilvent_dilF11_ero11_s1_sub3rdL_native is 0 - 2
#final outputs: _maxCLUSTERsize is the single largest contiguous cluster based on cluster size.
#               _largestCLUSTERsizes is a combination of the top larges clusters with sizes 10% - 100%
#NOTE: the largestCLUSTERsizes can be modified to include a different range of cluster sizes by changing the cutoff value below
...
##################################################################################################
#Trim:
fslmaths ${csf_dir}/${fname_short}_suptent_ero1_csf_native -mul ${restore_dir}/${fname_short}_restore_native_csf -bin ${csf_dir}/${fname_short}_suptent_ero1_csf_native_trimmed
fslmaths ${csf_dir}/${fname_short}_SA_csf_native -mul ${restore_dir}/${fname_short}_restore_native_csf -bin ${csf_dir}/${fname_short}_SA_csf_native_trimmed
